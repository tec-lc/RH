...
class BancoTxt {
    ...
    public function __construct($arquivo, $charset = 'utf8') {
        ...
        if (file_exists($arquivo)) {
            ...
            $this->xml = new SimpleXMLElement('<banco></banco>');
        }
    }
        ...
        foreach (@$this->xml->{$tabelas[0]}->registro as $registro1) {
            ...
            if (count($tabelas) > 1) {
                for ($i = 1; $i < count($tabelas); $i++) {
                    foreach ($this->xml->{$tabelas[$i]}->registro as $registro2) {
            ...
            array_unshift($resultado, $linha); // adiciona no início (ordem reversa)
        }
